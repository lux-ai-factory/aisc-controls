...
export default async function LibraryPage({ searchParams }: PageProps) {
  ...
  const [allRows, sources, ingest] = await Promise.all([
  ...
  return (
    <main className="page">
      <header className="page-header">
        <h1>Questionnaire library</h1>
        <p>
          Templates extracted from compliance documents. Filter by source,
      ...
      {rows.length === 0 ? (
        <div className="empty-state">
          <p>No questionnaires match the current filters.</p>
        </div>
      ) : (
          ...
          {rows.map((r) => (
            <article key={r.id} className="library-card">
              <div className="library-card-body">
                ...
                <p className="meta">
                  <Link
                    href={`/questionnaires?source=${encodeURIComponent(r.source.id)}`}
                ...
                <p className="page-tags">
                  {r.countryIds.map((c) => (
                    <span key={c} className="tag">
                      {countries.find((x) => x.id === c)?.name ?? c}
                    </span>
                  ))}
                  {r.regulationIds.map((rg) => (
                    <span key={rg} className="tag tag--reg">
                      {regulations.find((x) => x.id === rg)?.name ?? rg}
                    </span>
                  ))}
                </p>
                <p className="meta">
              ...
              <div className="library-card-actions">
                <Link className="btn" href={`/questionnaires/${r.id}/fill`}>
                  Fill it out
                </Link>
                <Link
                  className="btn ghost"
                  href={`/questionnaires/${r.id}/review`}
                >
                  Edit template
                </Link>
              </div>
